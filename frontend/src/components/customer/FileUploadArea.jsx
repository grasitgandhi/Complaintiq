  ...
  return (
    <div>
      <div
        ...
        style={{
          border: `2px dashed ${dragOver ? '#00B4A6' : '#D1D5DB'}`,
          borderRadius: 12, padding: '36px 24px', textAlign: 'center', cursor: 'pointer',
          background: dragOver ? '#F0FDFC' : '#F8F9FA', transition: 'all 0.2s',
        }}
      >
        <input id="ciq-file-input" type="file" multiple hidden accept=".pdf,.jpg,.png"
          onChange={e => [...e.target.files].forEach(addFile)} />
        <div style={{ fontSize: 32, marginBottom: 8 }}>📎</div>
        <p style={{ fontSize: 14, fontWeight: 600, color: '#374151', margin: 0 }}>Drag & drop or click to browse</p>
        <p style={{ fontSize: 12, color: '#9CA3AF', marginTop: 4 }}>PDF, JPG, PNG · Max 3 files · Max 5 MB each</p>
      </div>

      {files.length > 0 && (
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {files.map((f, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F8F9FA', borderRadius: 10, padding: '10px 14px' }}>
              <span>📄</span>
