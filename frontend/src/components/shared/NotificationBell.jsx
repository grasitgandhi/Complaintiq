      ...
      {open && (
        <>
          ...
          <div style={{
            ...
            background: isDark ? '#0F172A' : '#fff', borderRadius: 14, boxShadow: '0 8px 32px rgba(0,0,0,0.15)',
            zIndex: 99, border: isDark ? '1px solid #334155' : '1px solid #E5E7EB', overflow: 'hidden',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: isDark ? '1px solid #334155' : '1px solid #F3F4F6' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: isDark ? '#E2E8F0' : '#0A1628' }}>Notifications</span>
            ...
            {notifs.map(n => (
              <div key={n.id} style={{
                padding: '12px 16px', borderBottom: isDark ? '1px solid #1E293B' : '1px solid #F9FAFB',
                background: n.unread ? (isDark ? 'rgba(0,180,166,0.12)' : '#F0FDFC') : (isDark ? '#0F172A' : '#fff'),
              }}>
                <p style={{ fontSize: 12, color: isDark ? '#E2E8F0' : '#0A1628', margin: 0, lineHeight: 1.5 }}>{n.text}</p>
