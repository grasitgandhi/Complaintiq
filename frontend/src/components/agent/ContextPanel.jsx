...
export default function ContextPanel({ complaint, onUseResolution }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 4, background: '#F8F9FA', borderRadius: 10, padding: 4, marginBottom: 16, flexShrink: 0 }}>
        {TABS.map(t => (
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {tab === 'Customer' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: '#0A1628' }}>Customer Profile</span>
              <span style={{
            ...
            {[
              ...
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #F3F4F6' }}>
                ...
                <span style={{ fontSize: 12, color: '#0A1628', fontWeight: 600 }}>{v}</span>
              </div>
            ))}
            <div>
              <span style={{ fontSize: 12, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Open Products</span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {(complaint.open_products || [complaint.product_category]).map(p => (
                  <span key={p} style={{ background: '#F3F4F6', color: '#374151', borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 600 }}>{p}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {tab === 'Similar' && (
          <div>
            ...
            {[
              ...
              <div key={i} style={{ background: '#F8F9FA', borderRadius: 10, padding: 12, marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: '#00B4A6' }}>{s.ref}</span>
                  <span style={{ fontSize: 11, color: '#9CA3AF' }}>{s.sim * 100}% match · {s.days}d resolved</span>
                </div>
                <p style={{ fontSize: 12, color: '#374151', margin: '0 0 8px', lineHeight: 1.5 }}>{s.summary}</p>
                <button
        ...
        {tab === 'Policy' && (
          <div>
            ...
            {(complaint.ai_draft_policy_sources?.length > 0
              ...
              <div key={i} style={{ background: '#F8F9FA', borderRadius: 10, padding: 12, marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#0A1628' }}>{p.doc_name}</div>
                <div style={{ fontSize: 11, color: '#00B4A6', marginBottom: 4 }}>{p.section}</div>
                <p style={{ fontSize: 12, color: '#374151', lineHeight: 1.6, margin: 0 }}>{p.excerpt}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === 'CRM' && (
          <div>
            <h4 style={{ fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 10 }}>Recent Transactions</h4>
            {[
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F3F4F6' }}>
                <div>
                  <div style={{ fontSize: 12, color: '#0A1628', fontWeight: 600 }}>{tx.type}</div>
                  <div style={{ fontSize: 11, color: '#9CA3AF' }}>{tx.date}</div>
                </div>
                <span style={{ fontSize: 12, color: '#374151', fontWeight: 600 }}>{tx.amount}</span>
              </div>
            ))}
            <h4 style={{ fontSize: 12, fontWeight: 700, color: '#374151', margin: '16px 0 10px' }}>Recent Interactions</h4>
            {[
              ...
              <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid #F3F4F6' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 11, color: '#9CA3AF' }}>{ix.date}</span>
                </div>
                <p style={{ fontSize: 12, color: '#374151', margin: '4px 0 0' }}>{ix.note}</p>
              </div>
            ))}
          </div>
        )}
