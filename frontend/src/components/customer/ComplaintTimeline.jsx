...
export default function ComplaintTimeline({ events }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      {sorted.map((ev, i) => {
        const style = ACTOR_STYLE[ev.type] || ACTOR_STYLE.SYSTEM;
        return (
          <div key={i} style={{ display: 'flex', gap: 14, paddingBottom: 20, position: 'relative' }}>
            ...
            <div style={{ flex: 1, paddingTop: 4 }}>
              <p style={{ fontSize: 13, color: '#0A1628', margin: 0, lineHeight: 1.5 }}>{ev.text}</p>
              <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 3 }}>{fmtDateTime(ev.at)}</p>
            </div>
          </div>
        );
      })}
